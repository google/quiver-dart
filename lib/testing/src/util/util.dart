...
class EqualsTester {
  ...
  void _testItems() {
    var flattened = _equalityGroups.fold([], (prev, group) => prev..addAll(group));
    for (var item in flattened) {
      expect(_NotAnInstance.EQUAL_TO_NOTHING, isNot(equals(item)), reason: "$item must not be Object#equals to an arbitrary object of another class");
      expect(item, equals(item), reason: "$item must be Object#equals to itself");
      expect(item.hashCode, equals(item.hashCode), reason: "the Object#hashCode of $item must be consistent");
    }
  }
}
...
class _RelationshipTester<T> {
  ...
  void test() {
    for (var groupNumber = 0; groupNumber < _groups.length; groupNumber++) {
      ...
      for (var itemNumber = 0; itemNumber < group.length; itemNumber++) {
        ...
        for (var relatedItemNumber = 0; relatedItemNumber < group.length; relatedItemNumber++) {
        ...
        for (var unrelatedGroupNumber = 0; unrelatedGroupNumber < _groups.length; unrelatedGroupNumber++) {
          if (groupNumber != unrelatedGroupNumber) {
            var unrelatedGroup = _groups[unrelatedGroupNumber];
            for (var unrelatedItemNumber = 0; unrelatedItemNumber < unrelatedGroup.length;
                unrelatedItemNumber++) {
              _expectUnrelated(groupNumber, itemNumber, unrelatedGroupNumber, unrelatedItemNumber);
            }
          }
        }
      }
  ...
  void _expectUnrelated(int groupNumber, int itemNumber, int unrelatedGroupNumber,

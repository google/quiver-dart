...
/// Takes a [day] that may be outside the allowed range of dates for a given
...
///return the day which is xth last [weekDay] from yth [week] of zth [month] of a [year]
///eg. DateTime.Friday from 2nd last week of DateTime.july of a 2022 year.
/// [weekDay] ranges from 1 to 7. 1 = monday and 7 = sunday.
/// [week] ranges from 1 to 4 or 5 month depending upon the month.
/// [month] ranges from 1 to 12. 1 is january and 12 is december.
/// [year] the year in which you are calculating
/// if the number of week is more than the weeks in specific month day will be from another month
DateTime lastNthDayOnNthWeekOfAMonth(
  int weekDay,
  int week,
  int month,
  int year,
) {
  final lastDateOfMonth = DateTime(year, month, daysInMonth(year, month));

  return lastDateOfMonth.subtract(
    Duration(
      days: (7 - weekDay) +
          7 * (week - 1) -
          (lastDateOfMonth.weekday < weekDay ? 7 : 0),
    ),
  );
}
